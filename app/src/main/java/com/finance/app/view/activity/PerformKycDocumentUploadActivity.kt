...
class PerformKycDocumentUploadActivity : BaseAppCompatActivity() {
    ...
    private fun setOnClickListener() {
        ...
        binding.btnPickFile.setOnClickListener {
            if(binding.etDocumentName.text.toString().exIsNotEmptyOrNullOrBlank()){
                ...
                bundle.putString(Constants.KEY_MODULE_NAME , "KYCDocument")
    ...
    private fun getDocumentStatus(documentList: ArrayList<DocumentTypeModel>){
            ...
            val view = LayoutInflater.from(this).inflate(R.layout.uploaded_document_tracking, null)
        ...
        for(i in 0 until documentList.size){
            if(documentList[i].documentSubTypeDetailDisplayText == "Original KYC")
            {
                var alpha = 0.5
            ...
            if(documentList[i].documentSubTypeDetailDisplayText == "Customer Selfie with KYC"){
            ...
            if(documentList[i].documentSubTypeDetailDisplayText == "Customer self-attested KYC"){
                view_order_processed.setBackground(getResources().getDrawable(R.drawable.shape_status_completed));
                ready_divider.setBackgroundColor(resources.getColor(R.color.colorPrimary))
                processed_desc.text = "Uploaded Successfully"
        ...
        override fun getApiSuccess(value: Response.ResponseDocumentList) {
            ...
            if(value.responseObj!= null)
            {
                ...
                for(i in 0 until value.responseObj?.documentTypes!!.size)
                {
                    ...
                    // For UAT use below
                    if(value.responseObj?.documentTypes[i].documentSubTypeDetail == "OriginalKyc")
                    {
                        documentTypestemp.add(value.responseObj.documentTypes[i])

                    }
                    else if(value.responseObj?.documentTypes[i].documentSubTypeDetail == "CustomerSelfieWithKyc")
                    {
                        documentTypestemp.add(value.responseObj.documentTypes[i])
                    }
                    else if(value.responseObj?.documentTypes[i].documentSubTypeDetail == "CustomerSelf-attestedKyc")
                    {
                        documentTypestemp.add(value.responseObj.documentTypes[i])
                    }
                    //For production user below
